...
# # MODEL_PATH = os.getenv("MODEL_PATH", "C:/Users/Hirushi Silva/Documents/ExpliUI_Project (FYP)/backendV2/backendV2/best_v5_FINAL.pt")
# MODEL_PATH = os.getenv("MODEL_PATH", "C:/Users/Hirushi Silva/Documents/Main/ExpliUI/backendV2/best_v5_FINAL.pt")
# main_mod = sys.modules.get("__main__")
# if main_mod is not None:
#     setattr(main_mod, "SiameseEfficientNet", SiameseEfficientNet)
...
#             results.append({
#                 "pair_id": pair_id,
#                 "image1": predict_request.pair_list[0].image_list[0].image_name,
#                 "image2": predict_request.pair_list[0].image_list[1].image_name,
#                 "prediction": prediction,
#                 "rule_result": rule_result
#             })
...
def predict_result_by_image(predict_request: PredictRequest):
    ...
    for pair in predict_request.pair_list:
        ...
        if predict_request.type == 1:
            ...
            # type == 2 branch (URL flow or inline base64)
            # Save inline images if they are base64 and compare from files
...
transform = A.Compose([
    # Resize so the longest side is 512, keeping the aspect ratio
    A.LongestMaxSize(max_size=512),
    # Add padding to reach exactly 512x512
    A.PadIfNeeded(min_height=512, min_width=512, border_mode=0),
    A.Normalize(),
    ToTensorV2()
])
